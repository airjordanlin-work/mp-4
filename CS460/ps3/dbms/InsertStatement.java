...
public class InsertStatement extends SQLStatement {
    ...
    public void execute() throws DatabaseException, DeadlockException {
        try {
            ...
            DatabaseEntry k = new DatabaseEntry(row.getKeyBuffer().getBufferBytes());
            DatabaseEntry v = new DatabaseEntry(row.getValueBuffer().getBufferBytes());

            OperationStatus status = table.getDB().putNoOverwrite(null, k, v);

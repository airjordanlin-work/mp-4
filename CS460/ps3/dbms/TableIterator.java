...
public class TableIterator {
    ...
    public Object getColumnVal(int colIndex) {
        ...
        int offset = valIn.readShortAtOffset(colIndex * 2);

        if(col.isPrimaryKey())
        {

            if (col.getType() == Column.REAL){
                return valIn.readDoubleAtOffset(0); 
            }
            if(col.getType() == Column.INTEGER)
            {
                return valIn.readIntAtOffset(0); 
            }
            if(col.getType() == Column.CHAR || col.getType() == Column.VARCHAR)
            {
        ...
        {
            if (col.getType() == Column.REAL){
                return Double.valueOf(valIn.readDoubleAtOffset(offset));
            }
            if(col.getType() == Column.INTEGER)
            {
                return Integer.valueOf(valIn.readIntAtOffset(offset));
            }
            if(col.getType() == Column.CHAR)
            {
                return valIn.readBytesAtOffset(offset, col.getLength());
            }
            if(col.getType() == Column.VARCHAR)
            {
                int nextOffset = valIn.readShortAtOffset((colIndex + 1) * 2);
                return valIn.readBytesAtOffset(offset, nextOffset - offset);
            }
            else 
            {
                return null;
